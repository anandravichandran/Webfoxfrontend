  ...
  const onSignup = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        const errorMessage: string = errorData?.message || 'Registration failed';
        
  
        if (response.status === 409) {
          setErrors((prev) => ({ ...prev, general: 'User already exists' }));
          
          console.log("400:", formData);

        }
        else if (response.status === 505) {
          setErrors((prev) => ({ ...prev, general: 'server not exists' }));
         ;
          console.log("500:", formData);
        ...
        router.push(`/loginotp?email=${encodeURIComponent(formData.email)}`);
    ...
    <div className="w-screen h-screen overflow-hidden relative">
      ...
      <div className="h-screen w-full dark:bg-black-100 bg-white dark:bg-grid-white/[0.04] bg-grid-black/[0.2] flex items-center justify-center absolute top-0 left-0">
        ...
        <div className="bg-white-100 rounded-2xl shadow-black-100 shadow-2xl flex w-2/3 max-w-4xl ml-4 sm:ml-6 md:ml-6 lg:ml-10">
          <div className="w-3/5 rounded-tr-[10rem] rounded-br-[10rem] rounded-2xl dark:bg-violet-700 text-white py-36 px-12 flex flex-col justify-center">
